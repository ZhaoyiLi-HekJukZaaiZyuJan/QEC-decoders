...
Cluster::Cluster(const coord& S, const surfacetype& this_surf) : cluster(S, this_surf){
	vector<vector<int>> c_error_pos_211(3*S.x*S.y*S.z, vector<int>(2, 1));
	this->c_error_pos_211 = c_error_pos_211;
}

void Cluster::addPauli211(const double & p, const double & q, const int& seed){
...
void Cluster::addGateNoise(const double & p, const double & q, const int& seed){
	for (int c = 0; c < 3*S.x*S.y*S.z; c++) {
	...
	for (int c = 0; c < S.x*S.y*S.z; c++) {
		...
		for (int i = 0; i < 2; i++) {//211
			for (int face = 0; face < 3; face ++) {
				...
				if (p1z < p*2/3) {
					c_error_pos_211[C.getFaceQubits(S, face, 0, 3)][i] *= -1;					
				}
				...
				if (p2z < p*2/3) { //pink
					c_error_pos_211[C.getFaceQubits(S, face, 0, 2)][i] *= -1;
				}
				if (p2x < p*2/3/B) {
					c_error_pos_211[C.getFaceQubits(S, face, 0, 2)][i] *= -1;
					c_error_pos_211[C.getFaceQubits(S, face, 0, 3)][i] *= -1;
				...
				if (p3z < p*2/3) { //rose
					c_error_pos_211[C.getFaceQubits(S, face, 0, 1)][i] *= -1;
				}
				if (p3x < p*2/3/B) {
					c_error_pos_211[C.getFaceQubits(S, face, 0, 1)][i] *= -1;
					c_error_pos_211[C.getFaceQubits(S, face, 0, 2)][i] *= -1;
				...
				if (p4z < p*2/3) { //purple
					c_error_pos_211[C.getFaceQubits(S, face, 0, 0)][i] *= -1;
				}
...
//			c_error_pos_211[c][0] *= -1;
//		}
		c_loss_pos[c] = xx;//discard qubit if bad
