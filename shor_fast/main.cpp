...
class cluster {
	//422 structures
	vector<vector<int>> z_error_pos_sub;
...
public:
	...
	void addNoise(const double&, const noisemodel, const int&, const int&);
	void addGateNoise(const double&, const int&, const int&);
	
	void subdecode(const concattype&, int, int);
	void print();
...
cluster::cluster(const coord& S, const surfacetype& this_surf, const int& cp){
	this->S = S;
...
//	this->surf = surf;
	...
	vector<vector<int>> z_error_pos_sub(3*S.x*S.y*S.z, vector<int>(cp, 1));
	this->z_error_pos_sub = z_error_pos_sub;
}
void cluster::addNoise(const double & p, const noisemodel N, const int& cp, const int& seed=0){
...
void cluster::addGateNoise(const double & p, const int& cp, const int& seed=0){
	//Total heuristic Probabilities
	random_device rd;
	mt19937 engine{rd()};
	if (seed != 0) {
	...
	for (int c = 0; c < 3*S.x*S.y*S.z; c++) {
		for (int k = 0; k < cp; k++) {
			if(dist(engine) < p*4/3*(1-p*2/3)) {//measurement and storage errors
				z_error_pos_sub[c][k] = -1;
			} else {
				z_error_pos_sub[c][k] = 1;
			}
	...
	for (int c = 0; c < S.x*S.y*S.z; c++) {
		...
		for (int i = 0; i < cp; i++) {
			for (int face = 0; face < 3; face ++) {
				...
				if (p*4/15 < p1 && p1 < p*8/15) { //black
					z_error_pos_sub[C.getFaceQubits(S, face, 0, 3)][i] *= -1;//3
				}
				
				if (p4 < p*8/15) { //purple
					if (C.z % 2 == 1) {//even layer
						z_error_pos_sub[C.getFaceQubits(S, face, 0, 0)][i] *= -1;//0
					} else {
						z_error_pos_sub[C.getFaceQubits(S, face, 0, 1)][i] *= -1;//1
					}
				}
				...
				if ((p2 < p*4/15 && p1 >= p*4/15) || (p1 < p*4/15 && p2 >= p*4/15)) {
					z_error_pos_sub[C.getFaceQubits(S, face, 0, 0)][i] *= -1;//0
					z_error_pos_sub[C.getFaceQubits(S, face, 0, 1)][i] *= -1;//1
					z_error_pos_sub[C.getFaceQubits(S, face, 0, 2)][i] *= -1; //2
				...
				if ((p3 < p*4/15) || (p*4/15 < p3 && p3 < p*8/15)) { //horizontal
					z_error_pos_sub[C.getFaceQubits(S, face, 0, 1)][i] *= -1;//1
				} else if ((p3 < p*4/15) || (p*4/15 < p3 && p3 < p*8/15)){
					z_error_pos_sub[C.getFaceQubits(S, face, 0, 0)][i] *= -1;//0
...
//		//initialization errors X (dual) //none-fault tolerant
//		for (int i = 0; i < 2; i++) {//422
//			for (int face = 0; face < 3; face ++) {
//				double pA1 = dist(engine); //Prep error
//				double pA2 = dist(engine); //CZ error
//
//				if (pA2 < p*4/15){ // CZ Errors
//					for(int k=0 ; k <4; k++){
//						z_error_pos_sub[C.getFaceQubits(S, face, 0, k)][i+2] *= -1;//0123
//					}
//				} else if (p*4/15 < pA2 && pA2 < p*8/15){
//					for(int k=0 ; k <4; k++){
//						z_error_pos_sub[C.getFaceQubits(S, face, 0, k)][i] *= -1;//0123
//					}
//				}
//			}
//		}
//
//		//initialization errors Z (primal) //something wrong here
//		for (int k =0; k<4; k++) {
//			for (int i = 0; i < 3; i = i + 2) {
//				for (int face = 0; face < 3; face ++) {
//					double pA1 = dist(engine); //Prep
//					double pA2 = dist(engine); //CZ error
//
//					if (pA1 < p*2/3){ // P errors
//						z_error_pos_sub[C.getFaceQubits(S, face, 0, k)][i+1] *= -1;//k
//					}
//
//					if (pA2 < p*4/15){ // CZ Errors 12
//						z_error_pos_sub[C.getFaceQubits(S, face, 0, k)][i+1] *= -1;//k
//					} else if (p*4/15 < pA2 && pA2 < p*8/15){ //34
//						z_error_pos_sub[C.getFaceQubits(S, face, 0, k)][i] *= -1;//k
//					}
//				}
//			}
//		}
...
void cluster::subdecode(const concattype& C, int cp, int verbose = 0){
	...
	if (C == ShorX || C == ShorZ) {
		for (int c = 0; c < 3*S.x*S.y*S.z; c++) {
			...
			if (C == ShorX) {
				z_error_pos[c] = 1;
				vector<int> num_flip_sub(cp,0);
				for (int k_sub = 0; k_sub < cp; k_sub++) {//pn
					for (int k = 0; k < cp; k++) {
						num_flip_sub[k] += (z_error_pos_sub[c][k_sub + cp * k] == -1 ? 1 : 0);
					}
				}
				for (int k = 0; k < cp; k++) {//pm
					if (num_flip_sub[k] > cp/2) {
						z_error_pos[c] *= -1;
				...
				int num_flip = 0;
				vector<int> num_flip_sub(cp,1);
				for (int k_sub = 0; k_sub < cp; k_sub++) {//pm
					for (int k = 0; k < cp; k++) {
						num_flip_sub[k] *= z_error_pos_sub[c][k_sub + cp * k];
					}
				}
				for (int k = 0; k < cp; k++) {//pn
					if (num_flip_sub[k] == -1) {
						num_flip ++;
					}
				}
		...
		for (int c = 0; c < 3*S.x*S.y*S.z; c++) {
			//stabalizer measurement
			int g1 = z_error_pos_sub[c][3]*z_error_pos_sub[c][4]*z_error_pos_sub[c][5]*z_error_pos_sub[c][6];
			int g2 = z_error_pos_sub[c][1]*z_error_pos_sub[c][2]*z_error_pos_sub[c][5]*z_error_pos_sub[c][6];
			int g3 = z_error_pos_sub[c][0]*z_error_pos_sub[c][2]*z_error_pos_sub[c][4]*z_error_pos_sub[c][6];
			
			if (g1 == 1 & g2 == 1 && g3 ==-1) {
				z_error_pos_sub[c][0] *= -1;
			} else if(g1 == 1 & g2 == -1 && g3 ==1){
				z_error_pos_sub[c][1] *= -1;
			} else if (g1 == 1 & g2 == -1 && g3 ==-1){
				z_error_pos_sub[c][2] *= -1;
			} else if (g1 == -1 & g2 == 1 && g3 ==1){
				z_error_pos_sub[c][3] *= -1;
			} else if (g1 == -1 & g2 == 1 && g3 ==-1){
				z_error_pos_sub[c][4] *= -1;
			} else if (g1 == -1 & g2 == -1 && g3 ==1){
				z_error_pos_sub[c][5] *= -1;
			} else if (g1 == -1 & g2 == -1 && g3 ==-1){
				z_error_pos_sub[c][6] *= -1;
			}
			for (int k = 0; k < 7; k++) {
				z_error_pos[c] *= z_error_pos_sub[c][k];
//				loss_pos[c] *= z_error_pos_sub[c][k];
			}
		}
	} else if(C == S422){
		//422 code decoder
		for (int c = 0; c < 3*S.x*S.y*S.z; c++) {
			//stabalizer measurement
			int g1 = z_error_pos_sub[c][0]*z_error_pos_sub[c][1];
			int g2 = z_error_pos_sub[c][0]*z_error_pos_sub[c][2];
...
void testDecoding(cluster& test_cluster, const double& p, const double& pl, const int& seed, surfacetype surf, const concattype c, const int cp, noisemodel N, lossmodel Nl, int verbose){
	start_t = clock();
	//add noise
	if (N == GATE) {
...
int loopDecoding(const int L_min, const int L_max, const int trials, const double P_min, const double P_max, const int Np, const double Pl_min, const double Pl_max, const int Npl, const string fname, const surfacetype surf, const concattype c, const int cp, noisemodel N, lossmodel Nl, int verbose, bool thread, bool use_env){
	ofstream outfile;
	outfile.open(fname);
	...
	for (int il = 0; il <= Npl; il ++) {
		...
		for (int L = L_min; L <= L_max; L = L+2) {
			int M = L;
			cout << endl;
			cluster test_cluster({L,L,L,0}, surf, cp);
			for (int i = 0; i <= Np; i ++) {
				...
				if (thread) {
					parallel_for(trials, [&](int start, int end){
						...
						for(int i = start; i < end; ++i){
							if (N == GATE) {
								test_cluster.addGateNoise(p, cp, 0);
							} else {
								test_cluster.addNoise(p, N, cp, 0);
							}
							test_cluster.subdecode(c, cp);
							try {
								test_cluster.getSuperChunks();
...
int main(int argc, const char *argv[]) {
	...
	int L_min, L_max, n, Np, Npl, seed, times, cp;
	...
	("plmin", "Minimal loss probability", cxxopts::value(Pl_min)->default_value("1"))
	("plmax", "Maximal loss probability", cxxopts::value(Pl_max)->default_value("1"))
	
	("cp", "concatenation parameter", cxxopts::value(cp)->default_value("3"))
	("c", "concatenation type", cxxopts::value(c)->default_value("Steane"))
	...
	if (c == ShorX || c== ShorZ) {//change to squared
		cp = cp*cp;
	} if (c == Steane) {
		cp = 7;
	}
	...
	if (test) {
		cluster test_cluster({L_min,L_min,L_min,0}, s, cp);
		int i = 0;
		do {
			testDecoding(test_cluster, P_min, Pl_min, seed, s, c, cp, N, Nl, verbose);
			i++;
		}
		while (i < times);
	} else{
		return_value = loopDecoding(L_min, L_max, n, P_min, P_max, Np-1, Pl_min, Pl_max, Npl-1, fname, s, c, cp, N, Nl, verbose, thread, use_env);
	}
	return return_value;
}
...
//loop run
	...
	//(GATE)
		///./simulate -N GATE --pmin 0 --pmax 0.008  -n 1000 --Lmin 3 -v 1 ###(pE ~0.0?)
