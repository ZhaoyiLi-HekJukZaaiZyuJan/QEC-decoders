//########################## Planar Concatenated Toric Code ##########################
...
class cluster {
	//422 structures
	vector<subcluster> z_error_pos_sub;
...
//	this->surf = surf;
	...
	subcluster a_subcluster({cp,cp,0},subPLANE);

	vector<subcluster> z_error_pos_sub(3*S.x*S.y*S.z, a_subcluster);
...
void cluster::addGateNoise(const double & p, const int& cp, const int& seed=0){
	...
	for (int c = 0; c < S.x*S.y*S.z; c++) {
		...
		for (int i = 0; i < 2*cp*cp; i++) {
			for (int face = 0; face < 3; face ++) {
				...
				if (p*4/15 < p1 && p1 < p*8/15) { //black
					z_error_pos_sub[C.getFaceQubits(S, face, 0, 3)].z_error_pos[i] *= -1;//3
				}
				
				if (p4 < p*8/15) { //purple
					if (C.z % 2 == 1) {//even layer
						z_error_pos_sub[C.getFaceQubits(S, face, 0, 0)].z_error_pos[i] *= -1;//0
					} else {
						z_error_pos_sub[C.getFaceQubits(S, face, 0, 1)].z_error_pos[i] *= -1;//1
					}
				}
				...
				if ((p2 < p*4/15 && p1 >= p*4/15) || (p1 < p*4/15 && p2 >= p*4/15)) {
					z_error_pos_sub[C.getFaceQubits(S, face, 0, 0)].z_error_pos[i] *= -1;//0
					z_error_pos_sub[C.getFaceQubits(S, face, 0, 1)].z_error_pos[i] *= -1;//1
					z_error_pos_sub[C.getFaceQubits(S, face, 0, 2)].z_error_pos[i] *= -1; //2
		...
		for (int i = 0; i < cp; i++){
			z_error_pos_sub[c].z_error_pos[cp*cp + cp*i] = 1;
		}
		for (int i = 0; i < cp; i++){
			z_error_pos_sub[c].z_error_pos[2*cp*cp - i - 1] = 1;
		}
...
void cluster::subdecode(){
	//initialization loss and z_error
	//Toric decoder
	
	for (int c = 0; c < 3*S.x*S.y*S.z; c++) {
		z_error_pos_sub[c].getx_vec();
		z_error_pos[c] = z_error_pos_sub[c].decodeWithMWPM();
	}
...
//	surfaceCorrect(pm, vertices_num, wrong_chunks, boundary_info, verbose);
	...
	for (int i = 0; i < S.y; i++){//errors on left plane
		for (int j = 0; j < S.z; j++){
...
void testDecoding(cluster& test_cluster, const double& p, const double& pl, const int& seed, surfacetype surf, const int cp, noisemodel N, lossmodel Nl, int verbose){
	start_t = clock();
	//add noise
	if (N == GATE) {
		test_cluster.addGateNoise(p, cp, seed);
	} else {
		test_cluster.addNoise(p, N, seed);
	}
	
	test_cluster.subdecode();
...
int loopDecoding(const int L_min, const int L_max, const int trials, const double P_min, const double P_max, const int Np, const double Pl_min, const double Pl_max, const int Npl, const string fname, const surfacetype surf, const int cp, noisemodel N, lossmodel Nl, int verbose, bool thread, bool use_env){
	ofstream outfile;
	outfile.open(fname);
...
int main(int argc, const char *argv[]) {
	...
	if (test) {
		...
		int i = 0;
		do {
			testDecoding(test_cluster, P_min, Pl_min, seed, s, cp, N, Nl, verbose);
			i++;
		}
		while (i < times);
	} else{
		return_value = loopDecoding(L_min, L_max, n, P_min, P_max, Np-1, Pl_min, Pl_max, Npl-1, fname, s, cp, N, Nl, verbose, thread, use_env);
	}
	return return_value;
}
