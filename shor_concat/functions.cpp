...
istream& operator>> (istream& is, concattype& c){
	string str;
	is >> str;
	if (str == "ShorX") c = ShorX;
	if (str == "ShorZ") c = ShorZ;
	if (str == "Steane") c = Steane;
	if (str == "S422") c = S422;
	else;
	return is;
}
string to_string(concattype& surf){
	if (surf == ShorX) return "ShorX";
	if (surf == ShorZ) return "ShorZ";
	if (surf == Steane) return "Steane";
	if (surf == S422) return "S422";
	return "";
}
