...
void subcluster::decodeWithNN(cppflow::model model, bool binary_output, int verbose = 0){
...
void testDecoding(cppflow::model model, const int L, const int M, const double p, const int seed, bool binary_output, bool make_corrections, bool decode_with_NN, subsurfacetype surf, noisemodel N, bool dir){
...
void generator(const int trials, const double P_min, const double P_max, int seed, const int Np, const string fname, subsurfacetype surf, noisemodel N, bool verbose, bool thread, bool use_env, bool binary_output, bool new_cluster){
	ofstream outfile;
	outfile.open(fname);
	...
	for (int i = 0; i < trials; i++) {
		...
		while(true){
			...
			for (int c = 0; c < 2*subcluster::S.x*subcluster::S.y; c++) {
				...
				//get adjacent vertices
				int cV1 = subcluster::subcoord(C.x,C.y,!C.l).hash(), cV2, cV3;
				if (!C.l) { //0 qubits
					cV2 = subcluster::subcoord(divmod(C.x+1, subcluster::S.x), C.y, 0).hash();
					cV3 = subcluster::subcoord(C.x, divmod(C.y-1, subcluster::S.y), 1).hash();
				} else {
					cV2 = subcluster::subcoord(C.x, divmod(C.y+1, subcluster::S.y), 0).hash();
					cV3 = subcluster::subcoord(divmod(C.x-1, subcluster::S.x), C.y, 1).hash();
				}
				if (testcluster.stabs[c]<0||testcluster.stabs[cV1]<0||testcluster.stabs[cV2]<0||testcluster.stabs[cV3]<0){
...
void generator2(const int trials, const int L_min, const double P_min, const double P_max, int seed, const int Np, const string fname, subsurfacetype surf, noisemodel N, bool verbose, bool thread, bool use_env, bool binary_output, bool new_cluster){
	ofstream outfile;
	outfile.open(fname);
...
int main(int argc, const char *argv[]) {
	...
	int L_min, L_max, verbose, n, Np, seed, rpt;
	bool test, thread, use_env, generate_mode, binary_output, make_corrections, new_cluster, decode_with_NN, dir;
	double P_max, P_min;
	subsurfacetype s;
	noisemodel N;
	...
	if (test) {
		...
		for(int i = 0; i < rpt; i++){
			testDecoding(model, L_min, L_min, P_min, seed + i, binary_output, make_corrections, decode_with_NN, s, N, dir);
		}
		...
		generator2(n, L_min, P_min, P_max, seed, Np-1, fname, s, N, verbose, thread, use_env, binary_output, new_cluster);
	} else {
		if (fname == "") {
...
//./simulate -s TORUS --pmin 0 --pmax 0.18  --Np 25 -n 1000 --Lmin 5 --Lmax 5 -v 1 -d ~/ML/ -m "model,L=5(7),layer=3x128,epochs=10000,p=" --fname test.out --dir 1
//./simulate -s TORUS --pmin 0 --pmax 0.12  --Np 25 -n 1000 --Lmin 3 --Lmax 20 -v 1 -d ~/ML/  --fname test.out
//./simulate -s TORUS --pmin 0.03 --pmax 0.03  --Np 10 --Lmin 10 --thread -v 1 --test --make_corrections -d ~/ML/ -m "model,L=5(7),layer=3x128,epochs=10000,p=0.03"
