...
void testDecoding(Cluster& test_cluster, const double& p, const double& q, const int& seed, surfacetype surf, noisemodel N, lossmodel L, int verbosity){
	...
	int parity = test_cluster.decodeWithMWPM(verbosity, 1, 1, surf);
...
int loopDecoding(const int lmin, const int lmax, const int trials, const double pmin, const double pmax, const int Np, const double qmin, const double qmax, const int Nq, const string fname, surfacetype surf, noisemodel N, lossmodel L, int verbosity, int thread){
				...
				if (thread) {
					parallel_for(trials, [&](int start, int end){
						...
						for(int i = start; i < end; ++i){
							...
							if (surf == PLANE && test_cluster.decodeWithMWPM(verbosity, 1, 1, surf) == 1) {
...
int main(int argc, const char *argv[]) {
	...
	if (test) {
		...
		return_value = loopDecoding(lmin, lmax, n, pmin, pmax, Np-1, qmin, qmax, Nq-1, fname, s, N, L, verbosity, thread);
	}
	return return_value;
}
